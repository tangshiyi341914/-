  ...
  <div v-if="user">
    <el-card class="box-card" style="width: 400px; margin-top: 30px">
      ...
      <div>
        ...
        >
          <el-button type="primary" @click="showUpdateUserInfoView"
            >修改信息</el-button
          >
...
export default {
  ...
  methods: {
    ...
    updateUserInfo() {
      this.putRequest("/info", this.user2).then((resp) => {
        if (resp) {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.putRequest("/pass", this.ruleForm).then((resp) => {
            if (resp) {
